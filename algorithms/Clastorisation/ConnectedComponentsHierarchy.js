function lanceWilliams(alpha_U, alpha_V, beta, gamma, distance_US, distance_VS, distance_UV) {
    return alpha_U * distance_US + alpha_V * distance_VS + beta * distance_UV + gamma * Math.abs(distance_US - distance_VS)
}
function HierarchyClusters(points, alpha_U, alpha_V, beta, gamma, numberOfClusters) {
    ...
    for (let i = 0; i < clusters.length; i++) {
        ...
        for (let j = 0; j < clusters.length; j++) {
            distanceOnceCluster.push(Point.distanceBetween(points[clusters[i]], points[clusters[j]]))
        }
    ...
    for (let t = 1; t < points.length - numberOfClusters; t++) {
        ...
        for (let i = 0; i < newDistanceBetweenClusters.length; i++) {
            ...
            newDistanceBetweenClusters[i].push(lanceWilliams(alpha_U, alpha_V, beta, gamma, distanceBetweenClusters[cluster_U][i], distanceBetweenClusters[cluster_V][i], distanceBetweenClusters[cluster_U][cluster_V]))
        }
        let cluster_WDistance = [];
        for (let i = 0; i < newDistanceBetweenClusters.length; i++) {
            cluster_WDistance.push(lanceWilliams(alpha_U, alpha_V, beta, gamma, distanceBetweenClusters[cluster_U][i], distanceBetweenClusters[cluster_V][i], distanceBetweenClusters[cluster_U][cluster_V]))
        }
